...
// bool UDEInventoryComponent::TransferItemFromInventory(UDEInventoryComponent* TargetInventory, const FName& ItemID,
...
// 	}
// 	// Copy the data
// 	FDEInventorySlot& TempItem = InventoryContents[TempIndex];
...
bool UDEInventoryComponent::RemoveItem_Implementation(const FName& ItemID, const int32& Quantity)
{
	...
	for (int32 Index = InventoryContents.Num() - 1; Index >= 0; Index--)
	{
		...
		FDEInventorySlot& Slot = InventoryContents[Index];
...
int32 UDEInventoryComponent::AddItem_Implementation(const FName& ItemID, const int32& Quantity, const float& Durability)
{
	if (Quantity <= 0)
	{
		return Quantity;
	}
	const FDEItemData* ItemData = GetItemData(ItemID);
	...
	for (FDEInventorySlot& Slot : InventoryContents)
	{
		if (Remaining <= 0)
		{
			break;
...
bool UDEInventoryComponent::TransferItem_Implementation(const FName& ItemID, const int32& Quantity,
                                                        const TScriptInterface<IDEInventoryInterface>& TargetInventory)
{
	...
	for (int i = InventoryContents.Num() - 1; i >= 0; i--)
	{
		...
		TempIndex = i;
		FDEInventorySlot ExtractedItem;
...
bool UDEInventoryComponent::RemoveItemFromSlot_Implementation(const int32& Index, const int32& Quantity)
{
	FDEInventorySlot ExtractedItem;
...
int32 UDEInventoryComponent::AddItemToSlot_Implementation(const int32& Index, const FName& ItemID, const int32& Quantity,
...
bool UDEInventoryComponent::TransferItemFromSlot_Implementation(const int32& Index, const int32& Quantity,
	const TScriptInterface<IDEInventoryInterface>& TargetInventory)
{
...
bool UDEInventoryComponent::ExtractItemFromSlot(int32 Index, int32 Quantity, FDEInventorySlot& OutExtracted)
{
...
const FDEItemData* UDEInventoryComponent::GetItemData(FName ItemID) const
{
	if (ItemID.IsNone() || !ItemDataTable)
	{
		return nullptr;
	}
	return ItemDataTable->FindRow<FDEItemData>(ItemID, TEXT("GetItemData"), false);
}
